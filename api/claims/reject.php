<?php
/**
 * POST { claim_id, reason } - Reject claim (admin).
 */
...
$rejectedBy = $_SESSION['employee_id'] ?? $_SESSION['user_id'] ?? null;
// approved_by column is INT: use only numeric value or NULL (session may store string ID e.g. H3-2025-02-...)
$rejectedBy = is_numeric($rejectedBy) ? (int) $rejectedBy : null;
if (!$claimId || !isset($db)) {
...
$db->prepare("UPDATE claims SET status = 'rejected', approved_by = ?, approved_at = NOW() WHERE id = ?")->execute([$rejectedBy, $claimId]);
echo json_encode(['success' => true, 'message' => 'Claim rejected. Employee has been notified.']);
